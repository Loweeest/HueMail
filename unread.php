...
// Define folder as 'unread' (We're on the Unread folder)
$folder = 'unread'; 
...
if ($folder === 'unread') {
    ...
    $stmt->bindValue(':emails_per_page', $limit, PDO::PARAM_INT);
    $stmt->execute();
} else {
    // Fetch emails for other folders (e.g., inbox, starred, sent, etc.)
    $stmt = $pdo->prepare('
        SELECT id, sender, subject, body, received_at, status, attachment
        FROM inbox_emails
        ...
        LIMIT :offset, :emails_per_page
    ');
    $stmt->execute([':user_id' => $user_id, ':status' => $folder, ':offset' => $offset, ':emails_per_page' => $limit]);
}

$emails = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch total number of unread emails for pagination
$total_query = $pdo->prepare("SELECT COUNT(*) FROM inbox_emails WHERE user_id = ? AND status = 'unread'");
$total_query->execute([$user_id]);
$total_unread = $total_query->fetchColumn();
$total_pages = ceil($total_unread / $limit);
...
/* Optional: On hover, make the background slightly darker */
.unread-email:hover {
...
<div class="side-panel">
    ...
    <div class="navigation">
        ...
        <a href="inbox.php" class="<?php echo $folder === 'inbox' ? 'active' : ''; ?>"><i class="fas fa-inbox"></i> Inbox</a>
        <a href="starred.php" class="<?php echo $folder === 'starred' ? 'active' : ''; ?>"><i class="fas fa-star"></i> Starred</a>
        <a href="unread.php" class="active"><i class="fas fa-envelope-open-text"></i> Unread</a>
        <a href="sent.php" class="<?php echo $folder === 'sent' ? 'active' : ''; ?>"><i class="fas fa-paper-plane"></i> Sent</a>
        <a href="draft.php" class="<?php echo $folder === 'draft' ? 'active' : ''; ?>"><i class="fas fa-file-alt"></i> Drafts</a>
        <a href="archive.php" class="<?php echo $folder === 'archive' ? 'active' : ''; ?>"><i class="fas fa-archive"></i> Archive</a>
        <a href="spam.php" class="<?php echo $folder === 'spam' ? 'active' : ''; ?>"><i class="fas fa-exclamation-triangle"></i> Spam</a>
        <a href="trash.php" class="<?php echo $folder === 'trash' ? 'active' : ''; ?>"><i class="fas fa-trash"></i> Trash</a>
    </div>
</div>
...
<div class="main-content">
    ...
    <table>
        ...
        <tbody>
    <!-- PHP to render email rows -->
    <?php
    if (count($emails) == 0) {
        echo '<tr><td colspan="4" class="no-emails-message">No unread emails found.</td></tr>';
    } else {
        foreach ($emails as $email) {
            // Add the 'unread-email' class if the email is unread
            $row_class = ($email['status'] === 'unread') ? 'unread-email' : '';
            echo '<tr class="' . $row_class . '">';
            echo '<td><a href="view_unread.php?id=' . $email['id'] . '">View</a></td>';
