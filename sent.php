...
// Define folder as 'sent' (We're on the Sent folder)
$folder = 'sent'; 
...
// SQL query to fetch sent emails with pagination and search
$sql = '
    ...
    WHERE e.user_id = ? AND e.status = "sent"
';
...
// Fetch total number of sent emails for pagination
$total_query = $pdo->prepare("SELECT COUNT(*) FROM emails e WHERE e.user_id = ? AND e.status = 'sent'");
$total_query->execute([$user_id]);
$total_sent = $total_query->fetchColumn();
$total_pages = ceil($total_sent / $limit);
...
<div class="main-content">
    ...
    <table>
        ...
        <tbody>
            ...
            <?php
            if (count($emails) == 0) {
                echo '<tr><td colspan="4" class="no-emails-message">No sent emails found.</td></tr>';
            } else {
                foreach ($emails as $email) {
                    echo '<tr>';
                    echo '<td><a href="view_sent.php?id=' . $email['id'] . '">View</a></td>';
