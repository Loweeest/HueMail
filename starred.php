...
// Define folder as 'starred'
$folder = 'starred'; // We're on the Starred folder
...
// SQL query to fetch starred emails with pagination and search
$sql = '
...
// Fetch total number of starred emails for pagination
$total_query = $pdo->prepare("SELECT COUNT(*) FROM starred_emails s JOIN emails e ON s.email_id = e.id WHERE s.user_id = ?");
$total_query->execute([$user_id]);
$total_starred = $total_query->fetchColumn();
$total_pages = ceil($total_starred / $limit);
...
<head>
 ...
 <!-- Link to Bootstrap JS (locally) -->
  ...
  body {
            ...
            background: url('<?php echo $current_background; ?>') no-repeat center center fixed;
...
<a href="starred.php" class="active">
