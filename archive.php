...
// Check if an email ID is passed to archive
if (isset($_GET['id'])) {
    $email_id = $_GET['id'];

    // Update the email status to 'archive'
    $stmt = $pdo->prepare("UPDATE emails SET status = 'archive' WHERE id = ? AND user_id = ?");
    $stmt->execute([$email_id, $user_id]);

    // Redirect back to the Archive page after archiving
    header("Location: archive.php"); // You can change this to redirect anywhere else
    exit;
}
...
// SQL query to fetch archived emails with pagination using named parameters
$sql = '
    ...
    WHERE e.user_id = :user_id AND e.status = "archive"
...
// Fetch total number of archived emails for pagination
$total_query = $pdo->prepare("SELECT COUNT(*) FROM emails e WHERE e.user_id = :user_id AND e.status = 'archive'");
$total_query->bindParam(':user_id', $user_id, PDO::PARAM_INT);
$total_query->execute();
$total_archives = $total_query->fetchColumn();
...
<i class="fas fa-arrow-left" id="toggle-icon" style="font-size: 1.5em; cursor: pointer; margin-right: 1em;"></i>
    ...
    <table>
        ...
        <tbody>
            <?php
            if (count($emails) == 0) {
                echo '<tr><td colspan="4" class="no-emails-message">No archived emails found.</td></tr>';
            } else {
                foreach ($emails as $email) {
                    echo '<tr>';
                    echo '<td><a href="view_archive.php?id=' . $email['id'] . '">View</a></td>';
    ...
    <div class="pagination">
        ...
        // Pagination buttons
        if ($page > 1) {
            echo '<a href="?page=' . ($page - 1) . '&search=' . htmlspecialchars($search) . '">Prev</a>';
        }
        if ($page < $total_pages) {
            echo '<a href="?page=' . ($page + 1) . '&search=' . htmlspecialchars($search) . '">Next</a>';
        }
        ?>
    </div>
