...
// SQL query to fetch draft emails with pagination and search
$sql = '
    ...
    WHERE e.user_id = ? AND e.status = "draft"
';
...
// Fetch total number of draft emails for pagination
$total_query = $pdo->prepare("SELECT COUNT(*) FROM emails e WHERE e.user_id = ? AND e.status = 'draft'");
$total_query->execute([$user_id]);
$total_drafts = $total_query->fetchColumn();
...
<i class="fas fa-arrow-left" id="toggle-icon" style="font-size: 1.5em; cursor: pointer; margin-right: 1em; color: white"></i>
    ...
    <table>
        ...
        <tbody>
            <?php
            if (count($emails) == 0) {
                echo '<tr><td colspan="4" class="no-emails-message">No drafts found.</td></tr>';
            } else {
                foreach ($emails as $email) {
                    echo '<tr>';
                    echo '<td><a href="view_draft.php?id=' . $email['id'] . '">View</a></td>';
